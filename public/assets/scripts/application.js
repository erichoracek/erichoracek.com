...
scrollTo = function(to, duration, easingFunction, completion) {
  ...
  scroll = function(timestamp) {
    ...
    var scrollTop = easedT * (to - from) + from;
    root.scrollTop = scrollTop
    if (t < 1) {
      window.requestAnimationFrame(scroll);
    }
    if (t === 1 && completion !== undefined) {
      return completion();
    }
  };
  ...
  if (from !== to) {
    ...
    return window.requestAnimationFrame(completion);
  }
};
...
for (j = 0, len = scroll_links.length; j < len; j++) {
  ...
  for (k = 0, len1 = ref.length; k < len1; k++) {
    ...
    element.addEventListener("click", function(e) {
      ...
      e.preventDefault();
      scrollTo(attribute.offsetTop, 1000, easeOutFunction);
      return true;
    });
  }
}
...
document.querySelector('.nav-contact-link').addEventListener("click", function(e) {
  return scrollTo(0, 1000, easeOutFunction, function() {
